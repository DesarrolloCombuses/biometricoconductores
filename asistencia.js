...
async function submitAttendance(event) {
  ...
  try {
    ...
    const payload = {
      ...
      punto_operativo: state.isDriverCandidate ? location.punto_operativo || null : null,
      ubicacion_precision_m: location.precision || null,
      origen: state.isAdmin ? "admin_form" : "web",
      registrado_por: state.user.id,
      observacion: [userObservation, faceObservation, driverObservation].filter(Boolean).join(" | ") || null,
      enviado_buk: true,
